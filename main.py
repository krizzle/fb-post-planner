...
FACEBOOK_APP_ID = "332766106902973"
FACEBOOK_APP_SECRET = "41da18e579abc5842ae9a5411f77083e"
...
def write_template(self,template,template_values=None):
    header = JINJA_ENVIRONMENT.get_template('templates/top.html')
...
class PostToFBHandler(webapp2.RequestHandler):
    def post(self):
        ...
        if(content.get("id")):
           self.response.write('<script>alert("Done posting.");window.location.assign("/")</script>')
...
class DeleteHandler(webapp2.RequestHandler):
    def get(self,id):
        post = Posts.get_by_id(long(id))
        post.key.delete()
        self.response.write("<script> alert('Post Cancelled.');window.location.assign('/list/"+post.user_id+"')</script>")
